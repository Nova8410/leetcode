...
public:
  ...
  void pushBack(const T &value) {
    ...
    if (isEmpty()) {
      ...
      tail->next = newNode; // connect tail to new element
      tail = newNode;       // change tail to the added one
    }
    count++;
  }
  ...
  T popFront() {
    ...
    head = head->next;
    if (head == nullptr)
      tail = nullptr; // if there is only 1 element
    delete temp;
    count--;
    return value;
  }
  ...
  T popBack() {
    ...
    // Don't forget to handle the empty list case
    // Note: This is O(n) for singly linked list since you need to find the node
    // before tail
    if (isEmpty())
      throw underflow_error("List underflow");
    Node *temp = tail;
  ...
  void insert(int position, const T &value) {
    ...
    // Don't forget to handle special cases (position 0, end of list, invalid
    // position)
    ...
    if (position == 0) {
      ...
      for (int i{0}; i < position - 1; i++) {
        current = current->next;
      } // go to node before node to insert
      Node *newNode = new Node(value);
  ...
  void remove(int position) {
    ...
    // Don't forget to handle special cases (position 0, last element, invalid
    // position)
    if (position < 0 || position >= count) {
      throw out_of_range("Invalid position");
    } else if (position == 0) {
...
public:
  ...
  T popFront() {
    ...
    T value = temp->data;
    head = head->next;
    if (head == nullptr) // only 1 element
      tail = nullptr;
    else {
      head->prev = nullptr;
    }
  ...
  void insert(int position, const T &value) {
    ...
    if (position < 0 || position > count) {
      ...
      for (int i{0}; i < position - 1; i++) {
        current = current->next;
      } // before node to insert
      newNode->next = current->next;
      newNode->prev = current;
      current->next->prev = newNode;
      current->next =
          newNode; // adjust the 4 links between newNode and cur and cur->next
      count++;
    }
  }
...
template <typename T> void testLinkedLists() {
  // Test Singly Linked List
  std::cout << "Testing Singly Linked List:\n";
  SinglyLinkedList<T> singlyList;

  std::cout << "Is empty: " << (singlyList.isEmpty() ? "true" : "false")
            << std::endl;
  ...
  std::cout << "Is empty: " << (doublyList.isEmpty() ? "true" : "false")
            << std::endl;
