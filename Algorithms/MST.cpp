...
public:
  ...
  static int primsAlgorithm(Graph &graph) {
    ...
    while (!pq.empty()) {
      ...
      visited[u] = true;
      mstWeight += weight;
      for (const auto &[v, neighborW] : graph.adj[u]) {
        if (!visited[v])
          pq.push({neighborW, v});
      }
    }
  ...
  static int kruskalsAlgorithm(Graph &graph) {
    ...
    //    b. Find the set of u (call it rootU) and the set of v (call it rootV)
    //    using `dsu.find()`. c. If `rootU` is NOT equal to `rootV`:
