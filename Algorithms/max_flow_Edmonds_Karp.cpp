...
public:
  /**
   * Finds an augmenting path from source 's' to sink 't' in the 'residualGraph'
   * using BFS.
   * @param residualGraph The current residual capacity graph.
   * @param s The source node.
   * @param t The sink node.
   * @param parent An array to store the path found by BFS (parent[v] = u means
   * u comes before v).
   * @param V The number of vertices.
   * @return true if a path from s to t exists, false otherwise.
   */
  static bool bfs(std::vector<std::vector<int>> &residualGraph, int s, int t,
                  std::vector<int> &parent, int V) {
    ...
    //
    // 3. Enqueue the source node `s`, mark it as visited, and set its parent to
    // -1.
    ...
    //    c. If `v` is NOT visited AND there is residual capacity (i.e.,
    //    `residualGraph[u][v] > 0`):
    //       i. Enqueue `v`.
    //       ii. Mark `v` as visited.
    //       iii. Set `parent[v] = u`.
    //
    // 5. After the loop, return `visited[t]` (this will be `true` if we reached
  ...
  /**
   * Implements the Edmonds-Karp algorithm (a version of Ford-Fulkerson using
   * BFS) to find the maximum s-t flow.
  ...
  static int edmondsKarp(Graph &graph, int s, int t) {
    ...
    // after adding edges, residual graph holds u, v capacities
    ...
    //       - `u = parent[v];`
    //       - Decrease capacity on the forward edge: `residualGraph[u][v] -=
    //       pathFlow;`
    //       - Increase capacity on the backward edge: `residualGraph[v][u] +=
    //       pathFlow;`
    //
